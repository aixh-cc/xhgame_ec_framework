...
export abstract class BaseView extends Component implements IObserver {
    ...
    eventBusEmit(e: EventTouch, name: string = '') {
        name != '' && xhgame.event.emit(name)
    }
    /** 
     * 关闭窗口
     * */
    ...
    updateBySubject(modelComp: BaseModelComp) {
        ...
        for (let key of keys) {
            ...
            if (modelComp === this.viewModelComp) {
                // console.log('本modelComp内2的属性' + key)
                if (vals.indexOf(modelComp.compName + '::') > -1) {
                    vals = vals.replace(modelComp.compName + '::', '');
                }
                // 本model内的，不需要检测otherComp::attr这种格式
            } else {
                if (vals.indexOf(modelComp.compName + '::') === -1) {
